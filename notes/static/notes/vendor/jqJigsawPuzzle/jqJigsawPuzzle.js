...
/**
 * Width available for a puzzle inside its preview panel (0 = use natural image width).
 */
jqJigsawPuzzle.containerWidthForHost = function(host) {
    var block = host.closest('.puzzle-block');
    var parent = block.length ? block.parent() : host.parent();
    if (!parent.length) return 0;
    var parentW = parent.innerWidth();
    if (!parentW || parentW <= 0) return 0;
    var pad = 0;
    if (block.length) {
        pad = parseFloat(block.css('padding-left')) + parseFloat(block.css('padding-right'));
        if (isNaN(pad)) pad = 0;
    }
    return Math.max(120, Math.floor(parentW - pad - 2));
};
...
jqJigsawPuzzle.imageToPuzzle = function(imageSelector, options) {
    ...
    // Scale image to fit preview while keeping aspect ratio.
    var imgEl = img[0];
    var naturalW = imgEl.naturalWidth || img.width();
    var naturalH = imgEl.naturalHeight || img.height();
    if (!naturalW || !naturalH) return;
    var host = img.closest('.puzzle-jq-mount');
    if (!host.length) host = img.parent();
    var containerW = jqJigsawPuzzle.containerWidthForHost(host);
    var targetW = naturalW;
    if (containerW > 0 && naturalW > containerW) {
        targetW = containerW;
    }
    var targetH = Math.round(naturalH * (targetW / naturalW));
    img.css({ width: targetW, height: targetH, display: 'block', maxWidth: '100%' });

    var imgWidth = targetW;
    var imgHeight = targetH;
    host.css({ position: 'relative', overflow: 'visible', width: imgWidth + 'px' });
    var imgPosX = 0;
    var imgPosY = 0;
    var imgSrc = img.attr("src");
    var bgSize = imgWidth + 'px ' + imgHeight + 'px';
    img.hide();
    ...
    var html = '<div class="jigsaw" id="'+puzzleId+'" style="left:'+(imgPosX-borderWidth)+'px; top:'+(imgPosY-borderWidth)+'px; width:'+(imgWidth)+'px; min-height:'+(imgHeight)+'px; border-width:'+borderWidth+'px;">' +
                   '<div class="puzzle" style="width:'+imgWidth+'px; height:'+imgHeight+'px; background-image:url(\''+imgSrc+'\'); background-size:'+bgSize+'; background-repeat:no-repeat;"></div>' +
    ...
    // Calculate piece types (or reuse saved shapes when restoring state).
    var pieceTypes = (options != null && options.pieceTypes != null)
        ? options.pieceTypes
        : jqJigsawPuzzle.randomPieceTypes(rows, columns);
        ...
        for(var c=0; c<columns; c++) {
            ...
            jQuery("#" + id).draggable({
                ...
                stop: function(event, ui) {
                    ...
                    if (options != null && typeof options.onPieceStop === 'function') {
                        options.onPieceStop(jQuery(this), piecesContainer);
                    }
                }
            });
        }
    ...
    // Shuffle pieces (unless restoring a saved layout) and initialize counters.
    if (!(options != null && options.skipShuffle)) {
    ...
    jQuery("#" + puzzleId + "_shuffle").click(function() {
        ...
        jqJigsawPuzzle.shufflePieces(piecesContainer, options!=null? options.shuffle : null);
        jqJigsawPuzzle.resetCounters(piecesContainer);
        if (options != null && typeof options.onShuffle === 'function') {
            options.onShuffle(piecesContainer);
        }
    });

    if (options != null && typeof options.onReady === 'function') {
        options.onReady(piecesContainer);
    }
};
...
/**
 * Configure SoundManager (optional — skipped when library is not loaded).
 */
