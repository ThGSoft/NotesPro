...
(function (root, factory) {
  ...
  function evalSlotIndexedAssign(name, indexValues, exprAst, env, indexName) {
    ...
    for (const idxVal of indexValues) {
      const loopVars = indexName ? { [indexName]: cloneVal(idxVal) } : {};
      const pos = vectorSlotIndex(idxVal, origin);
      while (items.length <= pos) items.push(complex(0, 0));
      items[pos] = cloneVal(evalAst(exprAst, childEnv(env, loopVars)));
    }
  ...
  function evalScalarIndexedAssign(name, indexVal, exprAst, env) {
    return evalSlotIndexedAssign(name, [indexVal], exprAst, env, null);
  }
  ...
  function evalIndexedAssign(name, indexAst, exprAst, env) {
    if (indexAst.type !== 'id') {
      ...
      return evalSlotIndexedAssign(name, indexVal.items, exprAst, env, null);
    }
    ...
    return evalSlotIndexedAssign(name, idxVar.items, exprAst, env, indexName);
  }
  ...
  function evaluate(source, options = {}) {
    ...
    logicalLines.forEach((raw, idx) => {
      ...
      const exprSrc = raw.startsWith('=') ? raw.slice(1).trim() : raw;
  ...
  function renderPlotLegend(series, colors, W, pad) {
    ...
    const maxLabelLen = Math.max(...labels.map(l => l.length), 2);
    const boxW = boxPadX * 2 + swatchW + labelPad + maxLabelLen * 6.5;
    const boxH = boxPadY * 2 + series.length * rowH - 4;
    const boxX = W - pad.r - boxW;
    const boxY = pad.t + 4;
    const shadow = `<rect class="calcs-plot-legend-shadow" x="${(boxX + 3).toFixed(1)}" y="${(boxY + 3).toFixed(1)}" width="${boxW.toFixed(1)}" height="${boxH.toFixed(1)}" rx="2"/>`;
    const bg = `<rect class="calcs-plot-legend-box" x="${boxX.toFixed(1)}" y="${boxY.toFixed(1)}" width="${boxW.toFixed(1)}" height="${boxH.toFixed(1)}" rx="2"/>`;
    const items = series.map((s, i) => {
      const y = boxY + boxPadY + i * rowH + 9;
      const color = colors[i % colors.length];
      const label = labels[i];
      const lineX1 = boxX + boxPadX;
      const lineX2 = lineX1 + swatchW;
      const textX = lineX2 + labelPad;
      return [
        `<line class="calcs-plot-legend-swatch" x1="${lineX1.toFixed(1)}" y1="${y.toFixed(1)}" x2="${lineX2.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${color}"/>`,
        `<text class="calcs-plot-legend-label" x="${textX.toFixed(1)}" y="${(y + 3).toFixed(1)}">${escapeHtml(label)}</text>`,
      ].join('');
    }).join('');
    return `<g class="calcs-plot-legend">${shadow}${bg}${items}</g>`;
  }
  ...
  function renderPlotSvg(series, size, withMeta) {
    ...
    const legendPad = series.length ? 86 : 12;
    const pad = { l: 42, r: legendPad, t: 12, b: 28 };
    ...
    const legend = renderPlotLegend(series, colors, W, pad);
    const svg = [
  ...
  function plotDataToClient(svg, meta, x, y) {
    ...
    return {
      x: (pt.x / meta.w) * rect.width,
      y: (pt.y / meta.h) * rect.height,
    };
  }
